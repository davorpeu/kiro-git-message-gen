    ...
    try {
        ...
        await vscode.window.withProgress({
            ...
            const options = {
                ...
                maxLength: config.get<number>('maxSubjectLength', 50)
            };

            progress.report({ increment: 50, message: "Generating message with AI..." });

            // Generate the commit message
            const commitMessage = await commitMessageGenerator.generateMessage(options);
            ...
            switch (action) {
                case 'Copy to Clipboard':
                    await vscode.env.clipboard.writeText(commitMessage.subject);
                    vscode.window.showInformationMessage('Commit message copied to clipboard!');
                    break;
                
                case 'Insert into SCM':
                    await insertIntoSCM(commitMessage.subject);
                    break;
                
                case 'Edit Message':
                    await editCommitMessage(commitMessage.subject);
                    break;
            }
        });
        ...
        if (error.code === 'NO_STAGED_CHANGES') {
...
async function insertIntoSCM(message: string): Promise<void> {
    try {
        ...
        // Use the first repository (or could be enhanced to let user choose)
        ...
        vscode.window.showErrorMessage(`Failed to insert into SCM: ${error instanceof Error ? error.message : 'Unknown error'}`);
        // Fallback: copy to clipboard
        await vscode.env.clipboard.writeText(message);
        vscode.window.showInformationMessage('Message copied to clipboard as fallback');
    }
}
...
function initializeServices(context: vscode.ExtensionContext): void {
    try {
        ...
        const userPreferences: UserPreferences = {
            commitStyle: config.get<boolean>('enableConventionalCommits', true) ? 'conventional' : 'custom',
            includeBody: false, // Can be made configurable later
            customTypes: config.get<string[]>('customCommitTypes', ['feat', 'fix', 'docs', 'style', 'refactor', 'test', 'chore'])
        ...
        vscode.window.showErrorMessage(
            `Failed to initialize Git Commit Generator: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
    }
}
