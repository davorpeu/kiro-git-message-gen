...
import { GitService, GitDiff, ChangedFile, RepoStatus, ConflictInfo } from '../interfaces/GitService';
...
export class GitServiceImpl implements GitService {
    ...
    async getStagedChanges(): Promise<GitDiff> {
        try {
            ...
            const numstatLines = diffResult.split('\n').filter((line: string) => line.trim());
            
            for (const stagedFile of status.staged) {
                const numstatLine = numstatLines.find((line: string) => line.endsWith(stagedFile));
                let additions = 0;
                let deletions = 0;
    ...
    private async getFileDiff(filePath: string): Promise<string> {
        try {
            ...
            return `Error getting diff for ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`;
        }
    }

    /**
     * Determine the status of a file based on git status
     */
    private getFileStatus(filePath: string, status: StatusResult): "added" | "modified" | "deleted" | "renamed" {
     ...
     */
    private generateSummary(files: ChangedFile[], additions: number, deletions: number): string {
        ...
        const changeDescription = changeText.length > 0 ? ` (${changeText.join(', ')})` : '';
    ...
    async getConflictStatus(): Promise<ConflictInfo> {
        try {
            ...
            throw new Error(`Failed to get conflict status: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
}